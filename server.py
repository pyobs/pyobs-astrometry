...
def astrometry():
    ...
    # create table
    x = [c['x'] for c in data['cat']]
    y = [c['y'] for c in data['cat']]
    flux = [c['flux'] for c in data['cat']]
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # run astrometry.net
        try:
            output = subprocess.check_output([exec] + shlex.split(command), cwd=tmpdir, env={'PYTHONPATH': path})
            print(output)
        except subprocess.CalledProcessError:
            return {'error': 'Astrometry.net threw an error.'}

        # WCS file exists?
        if not os.path.exists(os.path.join(tmpdir, 'wcs.fits')):
            return {'error': 'Could not find WCS file.'}
    ...
    response = Response(json.dumps({'success': True, 'header': header}))
    response.content_type = 'application/json'
    return response
